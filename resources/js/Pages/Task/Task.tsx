    ...
    const columns = [
      ...
      },
      {
        ...
        childComponent: (value: any, index: number) => (
          <Fragment key={index}>
            <a onClick={() => router.get(route('task.show', 1))} className='group flex'><FaEye className='group-hover:underline mt-1 mr-1' /><p className='group-hover:underline'>View details</p></a>
          </Fragment>
        ),
        sort: false
      },
        ...
        >
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 grid space-y-5">
                  <PrimaryButton onClick={() => router.get(route('task.create'))} className='place-self-start'>Create</PrimaryButton>
